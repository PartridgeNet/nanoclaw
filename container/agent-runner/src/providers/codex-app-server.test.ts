...
describe('Codex config TOML', () => {
  ...
  it('emits the streamable-HTTP form for url-based servers and skips empty entries', () => {
    tmpHome = fs.mkdtempSync(path.join(os.tmpdir(), 'codex-home-'));
    process.env.HOME = tmpHome;

    writeCodexConfigToml({
      grub: { url: 'http://100.108.5.11:4180/mcp' },
      figma: {
        url: 'https://mcp.figma.com/mcp',
        bearer_token_env_var: 'FIGMA_TOKEN',
        http_headers: { 'X-Region': 'us-east-1' },
      },
      // stdio still works alongside HTTP entries
      local: { command: 'bun', args: ['run', 'server.ts'] },
      // neither url nor command — must be skipped entirely
      broken: {},
    });

    const content = fs.readFileSync(path.join(tmpHome, '.codex', 'config.toml'), 'utf-8');
    // Plain HTTP server: just url, no command.
    expect(content).toContain('[mcp_servers.grub]');
    expect(content).toContain('url = "http://100.108.5.11:4180/mcp"');
    // HTTP server with auth + headers.
    expect(content).toContain('[mcp_servers.figma]');
    expect(content).toContain('bearer_token_env_var = "FIGMA_TOKEN"');
    expect(content).toContain('[mcp_servers.figma.http_headers]');
    expect(content).toContain('X-Region = "us-east-1"');
    // stdio entry unaffected.
    expect(content).toContain('[mcp_servers.local]');
    expect(content).toContain('command = "bun"');
    // A url-based server never gets a spurious command line.
    expect(content).not.toContain('command = ""');
    // The entry with neither url nor command is omitted.
    expect(content).not.toContain('[mcp_servers.broken]');
  });
});
