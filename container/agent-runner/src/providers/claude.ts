...
function toClaudeSdkMcpServers(servers: Record<string, McpServerConfig>): Record<string, ClaudeSdkMcpServerConfig> {
  const out: Record<string, ClaudeSdkMcpServerConfig> = {};
  for (const [name, server] of Object.entries(servers)) {
    if (server.url) {
      const headers = { ...(server.http_headers ?? {}) };
      if (server.bearer_token_env_var) {
        const bearer = process.env[server.bearer_token_env_var];
...
export class ClaudeProvider implements AgentProvider {
  ...
  query(input: QueryInput): AgentQuery {
    ...
    const sdkMcpServers = toClaudeSdkMcpServers(this.mcpServers);
    ...
    const sdkResult = sdkQuery({
      ...
      options: {
        ...
        systemPrompt: instructions
          ...
          : undefined,
        allowedTools: [...TOOL_ALLOWLIST, ...Object.keys(sdkMcpServers).map(mcpAllowPattern)],
