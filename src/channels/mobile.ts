...
async function issueAccessToken(deviceId: string): Promise<{ accessToken: string; expiresAt: string }> {
  const accessToken = opaqueToken();
  const createdAt = now();
  const expiresAt = new Date(Date.now() + ACCESS_TOKEN_TTL_MS).toISOString();
  await getDb().run('DELETE FROM mobile_access_tokens WHERE expires_at <= ?', createdAt);
  await getDb().run(
...
async function authenticate(req: http.IncomingMessage): Promise<DeviceRow | null> {
  const value = req.headers.authorization;
  if (!value?.startsWith('Bearer ')) return null;
  const device = await getDb().get<DeviceRow>(
  ...
  if (device) await getDb().run('UPDATE mobile_devices SET last_seen_at = ? WHERE id = ?', now(), device.id);
  return device ?? null;
}
...
async function ensureConversation(deviceId: string, agentGroupId: string): Promise<Session> {
  ...
  await getDb().run(
    ...
    messagingGroupId,
    `${deviceId}|${agentGroupId}`,
    `NanoClaw Chat: ${agentGroupId}`,
    createdAt,
  );
  return (await resolveSession(agentGroupId, messagingGroupId, null, 'shared')).session;
}
...
async function pair(body: Record<string, unknown>, res: http.ServerResponse): Promise<void> {
  ...
  json(res, 201, { deviceId, refreshToken, ...(await issueAccessToken(deviceId)) });
}

async function refresh(body: Record<string, unknown>, res: http.ServerResponse): Promise<void> {
  const refreshToken = typeof body.refreshToken === 'string' ? body.refreshToken : '';
  const device = await getDb().get<DeviceRow>(
    'SELECT id, name, revoked_at FROM mobile_devices WHERE refresh_hash = ? AND revoked_at IS NULL',
    hash(refreshToken),
  );
  if (!device) {
  ...
  await getDb().run(
    'UPDATE mobile_devices SET refresh_hash = ?, last_seen_at = ? WHERE id = ?',
    hash(replacement),
    now(),
    device.id,
  );
  json(res, 200, { deviceId: device.id, refreshToken: replacement, ...(await issueAccessToken(device.id)) });
}
...
async function sendMessage(device: DeviceRow, body: Record<string, unknown>, res: http.ServerResponse): Promise<void> {
  ...
  const session = await ensureConversation(device.id, agentGroupId);
...
async function handleRequest(req: http.IncomingMessage, res: http.ServerResponse): Promise<void> {
  ...
  try {
    ...
    if (req.method === 'GET' && url.pathname === '/webhook/mobile/agents') {
      json(res, 200, {
        agents: (await getAllAgentGroups()).map(({ id, name }) => ({ id, name })),
      });
      return;
    }
    ...
    if (req.method === 'POST' && readMatch) {
      const result = await getDb().run(
        'UPDATE mobile_events SET read_at = COALESCE(read_at, ?) WHERE id = ? AND device_id = ?',
        now(),
    ...
    if (req.method === 'DELETE' && url.pathname === '/webhook/mobile/device') {
      await getDb().run('UPDATE mobile_devices SET revoked_at = ? WHERE id = ?', now(), device.id);
      await getDb().run('DELETE FROM mobile_access_tokens WHERE device_id = ?', device.id);
...
class MobileChannelAdapter implements ChannelAdapter {
  ...
  async deliver(platformId: string, _threadId: string | null, message: OutboundMessage): Promise<string> {
    ...
    await getDb().run(
      ...
      // OutboundMessage no longer carries senderName; delivery metadata lives in
      // the outbox row itself. Persist NULL — the event's content still carries
