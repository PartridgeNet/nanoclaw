...
const NATIVE_RUNTIME_SKILLS_POINTER = [
  ...
  'Skills YOU author or install yourself go in `/workspace/agent/skills/<name>/SKILL.md` — persistent, provider-neutral (they load under any agent provider this group runs on), and yours to write and update over time. They are linked into `.agents/skills` (alongside the shared skills) at each session start, so a skill you create becomes discoverable on your next session. Never write skills anywhere else: paths outside your workspace are ephemeral.',
].join('\n\n');
