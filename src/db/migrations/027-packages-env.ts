 ...
 *
 * NOTE: `name` is 'packages-env' — already applied under that name on the live
 * DB (authored as `020-packages-env`); renumbered to 027, name unchanged.
 */
export const migration027: Migration = {
  version: 27,
  name: 'packages-env',
  async up(db) {
    await db.exec(`ALTER TABLE container_configs ADD COLUMN packages_env TEXT NOT NULL DEFAULT '{}';`);
  },
};
