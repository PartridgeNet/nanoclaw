...
export function createSlackBridge(options: SlackBridgeOptions = {}): ChannelAdapter | null {
  ...
  const bridge = createChatSdkBridge({
    ...
    // PartridgeNet: show which agent group is replying in the Slack sender, e.g.
    // "Assistant [meal-planner]". Forwarded as chat.postMessage's `username`
    // override (requires the `chat:write.customize` bot scope) via the patched
    // @chat-adapter/slack. See docs/slack-agent-sender-name.md.
