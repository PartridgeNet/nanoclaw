...
export interface CodexMcpServer {
  // stdio transport: Codex spawns this command and talks JSON-RPC over its pipes.
  command?: string;
  args?: string[];
  env?: Record<string, string>;
  // Streamable HTTP transport: when `url` is set, Codex connects to the remote
  // endpoint instead of spawning a process, and the stdio fields are ignored.
  // Mirrors Codex's own `[mcp_servers.<name>]` HTTP form (url / bearer_token_env_var
  // / http_headers). NanoClaw's HTTP MCP auth normally rides the OneCLI gateway
  // proxy, so bearer_token_env_var / http_headers are only for servers that
...
export function writeCodexConfigToml(
  ...
  for (const [name, config] of Object.entries(servers)) {
    // `url` is the discriminator: a streamable-HTTP server has no command to
    // spawn. Skip entries that have neither rather than emit a broken table.
    if (config.url) {
      lines.push(`[mcp_servers.${name}]`);
      lines.push(`url = ${tomlBasicString(config.url)}`);
      if (config.bearer_token_env_var) {
        lines.push(`bearer_token_env_var = ${tomlBasicString(config.bearer_token_env_var)}`);
      }
      if (config.http_headers && Object.keys(config.http_headers).length > 0) {
        lines.push(`[mcp_servers.${name}.http_headers]`);
        for (const [key, value] of Object.entries(config.http_headers)) {
