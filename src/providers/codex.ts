...
/**
 * Sync `.agents/skills/<name>` symlinks — codex's skill-discovery directory.
 * Targets are container paths (dangling on the host, valid inside):
 *
 *   - shared skills        → `/app/skills/<name>`            (read-only mount)
 *   - the group's own
 *     self-authored skills → `/workspace/agent/skills/<name>` (the RW workspace)
 *
 * Without the second set, a skill the group authors under
 * `/workspace/agent/skills/<name>/` is invisible to codex (it only scans
 * `.agents/skills` and `$CODEX_HOME/skills`), so the agent reports it lacks a
 * capability it actually has on disk. On a name collision the group skill wins.
 */
function syncCodexSkillLinks(groupDir: string, selectedSkills: string[]): void {
  ...
  for (const skill of selectedSkills) targets.set(skill, `/app/skills/${skill}`);

  // The group's own skills live under the RW workspace and are mounted at
  // /workspace/agent/skills inside the container. Only directories carrying a
  // SKILL.md are codex skills.
  const groupSkillsDir = path.join(groupDir, 'skills');
  let groupEntries: string[] = [];
  try {
    groupEntries = fs.readdirSync(groupSkillsDir);
  } catch {
    groupEntries = [];
  }
  for (const name of groupEntries) {
    if (!fs.existsSync(path.join(groupSkillsDir, name, 'SKILL.md'))) continue;
    targets.set(name, `/workspace/agent/skills/${name}`);
  }

  // Prune stale links and any whose target has changed (e.g. a shared skill
  // shadowed by a new group skill of the same name).
  for (const entry of fs.readdirSync(skillsDir)) {
    ...
    if (targets.get(entry) !== link) fs.unlinkSync(entryPath);
  }
