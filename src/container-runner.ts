...
async function buildContainerArgs(
  ...
  // Git needs two things that can't be set as static docker -e flags:
  ...
  // GIT_SSL_CAINFO is exported so child processes inherit it. proxyAuthMethod is
  // written to ~/.gitconfig via `git config --global` (more reliable than
  // GIT_CONFIG_COUNT env injection, which requires git ≥ 2.32 and didn't work here).
  args.push(
    '-c',
    'git config --global http.proxyAuthMethod basic; [ -n "$SSL_CERT_FILE" ] && export GIT_SSL_CAINFO="$SSL_CERT_FILE"; exec bun run /app/src/index.ts',
  );
...
export async function buildAgentGroupImage(agentGroupId: string): Promise<void> {
  ...
  try {
    execSync(`${CONTAINER_RUNTIME_BIN} build --no-cache -t ${imageTag} -f ${tmpDockerfile} .`, {
      cwd: DATA_DIR,
      stdio: 'pipe',
      timeout: 900_000,
    });
