...
async function createA2aSession(agentGroupId: string, sourceSessionId: string, id: string): Promise<Session> {
...
describe('routeAgentMessage return-path', () => {
  ...
  it('forward direction: stamps source_session_id on the target inbound row', async () => {
    ...
    const bSession = await findA2aSession(B, S1.id);
  ...
  it('fallback: a2a with no in_reply_to creates an isolated per-source session', async () => {
    ...
    const s1Rows = readInbound(A, S1.id);
    const s2Rows = readInbound(A, S2.id);
    const isolatedRows = readInbound(A, (await findA2aSession(A, SB.id)).id);
  ...
  it('stale origin fallback: closed origin session falls through to a fresh isolated session', async () => {
    ...
    const s1Rows = readInbound(A, S1.id);
    const s2Rows = readInbound(A, S2.id);
    const isolatedRows = readInbound(A, (await findA2aSession(A, bSession.id)).id);
    expect(s1Rows).toHaveLength(0);
    expect(s2Rows).toHaveLength(0);
    expect(isolatedRows).toHaveLength(1);
  });
  ...
  it('cross-agent-group guard: origin session belonging to wrong agent group is rejected', async () => {
    ...
    const bSession = await findA2aSession(B, SC.id);
    const bRows = readInbound(B, bSession.id);
  ...
  it('self-message is allowed without a destination row', async () => {
    ...
    const selfRows = readInbound(A, (await findA2aSession(A, S1.id)).id);
  ...
  it('BUG: no volume cap on a2a routing — unbounded ping-pong is allowed (#2063)', async () => {
    ...
    for (let i = 0; i < 20; i++) {
      try {
        ...
        bSession = await findA2aSession(B, S1.id);
        await routeAgentMessage(
  ...
  it('file forwarding (#2828): skips a symlinked target inbox dir, writes nothing outside', async () => {
    ...
    const bSession = await createA2aSession(B, S1.id, 'sess-B-from-S1');
