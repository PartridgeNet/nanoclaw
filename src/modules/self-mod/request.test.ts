...
describe('add_mcp_server approval card', () => {
  ...
  it('supports remote HTTP servers and renders every applied HTTP field', async () => {
    await submitAddMcpServer(
      {
        name: 'remote',
        url: 'https://mcp.example.com/mcp',
        bearer_token_env_var: 'MCP_TOKEN',
        http_headers: { Authorization: 'Bearer secret-token', 'X-Trace': 'visible' },
      },
      session,
    );
    ...
    expect(question).toContain('https://mcp.example.com/mcp');
    expect(question).toContain('MCP_TOKEN');
    expect(question).toContain('Authorization');
    expect(question).toContain('X-Trace');
    expect(question).toContain('visible');
    expect(question).not.toContain('Bearer secret-token');
    ...
    expect(payload.bearer_token_env_var).toBe('MCP_TOKEN');
    expect(payload.http_headers.Authorization).toBe('Bearer secret-token');
  });
