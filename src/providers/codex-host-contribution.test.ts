...
describe('codex host contribution against real core', () => {
  ...
  it('links the group’s own self-authored skills into .agents/skills so codex can discover them', () => {
    const ag = group('ag-codex-skills', 'codex-skills-group');
    createAgentGroup(ag);
    ensureContainerConfig(ag.id);
    const groupDir = path.join(GROUPS_DIR, ag.folder);

    // A skill the group authored under its RW workspace skills dir.
    fs.mkdirSync(path.join(groupDir, 'skills', 'protonmail'), { recursive: true });
    fs.writeFileSync(path.join(groupDir, 'skills', 'protonmail', 'SKILL.md'), '# protonmail\n');
    // A stray dir without SKILL.md must be ignored (not a skill).
    fs.mkdirSync(path.join(groupDir, 'skills', 'notes'), { recursive: true });
    ...
    // Shared skill → /app/skills, group skill → /workspace/agent/skills.
    expect(fs.readlinkSync(path.join(skillsDir, 'onecli-gateway'))).toBe('/app/skills/onecli-gateway');
    expect(fs.readlinkSync(path.join(skillsDir, 'protonmail'))).toBe('/workspace/agent/skills/protonmail');
    // Non-skill directory is not linked.
