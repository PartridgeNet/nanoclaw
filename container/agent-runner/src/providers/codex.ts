...
// Per-turn cap for the whole codex turn (all the agent's tool calls + reasoning
// for one message). Kept safely below the host-sweep absolute ceiling (30 min,
// src/host-sweep.ts ABSOLUTE_CEILING_MS) so a long turn finishes on its own
// terms rather than being preempted by the sweep. Overridable via env for
// per-deployment tuning. Agents should still split large work across turns.
