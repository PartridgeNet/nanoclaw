...
| [docs/slack-agent-sender-name.md](docs/slack-agent-sender-name.md) | **PartridgeNet:** Slack sender shows `Assistant [<agent-group>]` — threaded `senderName`, the `@chat-adapter/slack` pnpm patch, the `chat:write.customize` scope, deploy + maintenance |
| [docs/live-browser.md](docs/live-browser.md) | **PartridgeNet:** Persistent headless Chromium service per agent group for logged-in browsing (anti-bot sites, Google session-bound auth). Architecture, port map, setup, and sign-in via `scripts/live-chrome-login.sh` |
| [docs/browser-session-export.md](docs/browser-session-export.md) | Log in to a site on the host, export the session (cookies + localStorage) into a group workspace; the agent's `agent-browser` reuses it. General "session export" browser-auth — `scripts/browser-session-export.sh`. Anti-bot sites (Tesco) still need the live-browser path |
